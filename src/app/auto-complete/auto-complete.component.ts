/**
 * This component is customized version of existing project - Typeahead - https://github.com/brinkmanjg/ng2-typeahead
 */
...
export class AutoCompleteComponent implements OnInit, ControlValueAccessor {
  ...
  public populateSuggestions() {
    ...
    let input = this.removeDiacritics(this.input);
    ...
    this.suggestions = this.list.filter((item) => {
      return this.removeDiacritics(item[searchProperty].toLowerCase()).indexOf(input.toLowerCase()) > -1;
    });
  ...
  removeDiacritics(text) {
    if(text) {
      let dia = "áäčďéíľĺňóôŕšťúýÁČĎÉÍĽĹŇÓŠŤÚÝŽ";
      let nodia = "aacdeillnoorstuyACDEILLNOSTUYZ";
